...
function App() {
  ...
  const handleBuyClick = () => {
    ...
    dispatch(addEntradas({ peliculaId: selectedMovieDetails.title, cantidad: numEntradas }));
  };
  ...
  const handleAddToFavorites = () => {
    dispatch(agregarPeliculaFavorita(selectedMovieDetails));
  };

  const handleRemoveFromFavorites = () => {
    dispatch(eliminarPeliculaFavorita(selectedMovieId));
  };
        ...
        <div>
          <div className='mt-5 ml-30 mr-30 mb-15' style={{ display: 'flex', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: '1' }}>
              ...
              <button className='mb-3 ml-3 border-2 border-green-500 px-4 py-2 rounded hover:bg-green-500 hover:text-white' onClick={handleBuyButtonClick}><b>Comprar</b></button>

              {peliculasFavoritas.some(movie => movie.id === selectedMovieDetails.id) ? (
                <button className='mb-3 ml-3 border-2 border-green-500 px-4 py-2 rounded hover:bg-green-500 hover:text-white' onClick={handleRemoveFromFavorites}><b>Eliminar de favoritos</b></button>
              ) : (
      ...
      <Modal isOpen={isModalOpen} onRequestClose={closeModal} style={{ content: { width: '300px', height: '300px', margin: 'auto' } }}>
        {isPurchaseSuccessful ? (
          ...
          <div>
            ...
            <select id="salas" className='mb-3'>
